...
os.environ['SDL_VIDEO_WINDOW_POS'] = "%d,%d" % (
    100, 100)  # this line lets you position the window on your screen
import random
import math
import pgzrun
...
DUCKSPEEDS = [1.5, -2, 2, -1]
# speeds for the ducks when they are spawned are chosen from this list
...
duckspeeds = random.sample(
    DUCKSPEEDS, len(DUCKSPEEDS))  # mix up the list of duckspeeds randomly
...
def draw():
  ...
  if gameStatus == 1:  # play the game
    ...
    screen.draw.text(str(score),
                     bottomright=(WIDTH - 50, HEIGHT - 20),
                     owidth=0.5,
                     ocolor=(255, 255, 255),
                     color=(0, 64, 255),
                     fontsize=48)
    ...
    drawDucks()
    hero.draw()
    screen.draw.text("YOUR SCORE:  " + str(score), (20, 30), color="blue")
    # ducklife=6
    # screen.draw.text("DUCK LIFE :  " + str(ducklife), (20, 60),color="red")
...
def drawDrones():
  for d in range(len(drones)):
    if drones[d].status > 0:
      drones[d].image = BOOM_IMAGES[math.floor(
          drones[d].status /
          10)]  # animation, change image every 10th iteration
    drones[d].draw()


def drawDucks():
  for a in range(len(ducks)):
    ducks[a].draw()
...
def writehighScores():
  ...
  if len(highScores) > 5:
    highScores.pop(
    )  # discard the lowest score if there are more than 5 scores in the list
  for s in highScores:
    hsFile.write(str(s) + "\n")
...
def update():
  ...
  if gameStatus == 1:
    ...
    if len(ducks) == 0:  # on game over read and write the highscores.txt file
      ...
      gameStatus = 2
    lasers = listCleanup(
        lasers)  # delete all destroyed Actors from their Actor lists
...
def findEmptySky(
):  # find an empty piece of sky to place a new drone in. returns skyFound, x en y
  attempt = 1
  while attempt < 30:
...
def listCleanup(l):
  ...
  for i in range(len(l)):
    if l[i].status < 30:
      newList.append(
          l[i]
      )  # a status of 30 or larger cleans the Actor from the list immediatel
...
def ducksCleanup(l):
  ...
  for i in range(len(l)):
    ...
    if l[i].status < 30:
      newList.append(l[i])
    if l[i].status >= 30:
      print("duck:", ducklives)
      ducklives -= 1
      # screen.draw.text("DUCKS:  " + str(ducklives), (50, 90),color="black")
...
def checkPlayerLaserHit(l):
  ...
  for d in range(len(drones)):
    if drones[d].collidepoint((lasers[l].x, lasers[l].y)):
      ...
      score += 10 * len(
          ducks
      )  # you get more points for shooting a drone with more ducks on screen
  for d in range(len(ducks)):
  ...
  for d in range(MAX_DUCKS):
    ...
    if s < 0:
      ...
      ducks.append(Actor(
          'ducklwingup',
          (-200, 100 +
           d * 120)))  # start the ducks here so they'll fly in from both sides
    else:
      ducks.append(Actor(
          'duckrwingup',
          (-200, 100 + d * 120)))  # the ducks are 120 px vertically separated
    ducks[d].floatx = float(ducks[d].x)
    ducks[d].floaty = float(ducks[d].y)
    ducks[d].velocity = s
    ducks[d].wingspeed = random.randint(
        10, 20)  # each duck flaps its wings at a randomly chosen frequency
    ducks[d].wingstatus = random.randint(
        0, 10)  # random start time for wings flapping
    ducks[d].status = 0
  clock.schedule(playTimeOver, MAX_PLAYTIME)
  # game over after MAX_PLAYTIME seconds to prevent brain damage
